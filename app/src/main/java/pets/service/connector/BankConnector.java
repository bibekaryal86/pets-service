...
public class BankConnector {
  ...
  public RefBankResponse getAllBanks() {
    ResponseEntity<RefBankResponse> responseEntity =
        restTemplate.getForEntity(getAllBanksUrl, RefBankResponse.class);
  ...
  public RefBankResponse saveNewBank(@NonNull final RefBankRequest refBankRequest) {
    ResponseEntity<RefBankResponse> responseEntity =
        restTemplate.exchange(
            saveNewBankUrl, POST, new HttpEntity<>(refBankRequest, null), RefBankResponse.class);

    return responseEntity.getBody();
  }

  @CacheEvict(value = "banks", allEntries = true, beforeInvocation = true)
  public RefBankResponse updateBank(
      @NonNull final String id, @NonNull final RefBankRequest refBankRequest) {
    String url = UriComponentsBuilder.fromHttpUrl(updateBankUrl).buildAndExpand(id).toString();

    ResponseEntity<RefBankResponse> responseEntity =
        restTemplate.exchange(
            url, PUT, new HttpEntity<>(refBankRequest, null), RefBankResponse.class);
  ...
  public RefBankResponse deleteBank(@NonNull final String id) {
    String url = UriComponentsBuilder.fromHttpUrl(deleteBankUrl).buildAndExpand(id).toString();

    ResponseEntity<RefBankResponse> responseEntity =
        restTemplate.exchange(url, DELETE, new HttpEntity<>(null, null), RefBankResponse.class);
