...
public class UserConnector {
  ...
  public UserResponse getUserByUsername(@NonNull final String username) {
    ...
    String url =
        UriComponentsBuilder.fromHttpUrl(getUserByUsernameUrl)
            .buildAndExpand(pathVariables)
            .toString();

    ResponseEntity<UserResponse> responseEntity =
        restTemplate.exchange(url, GET, new HttpEntity<>(null, httpHeaders), UserResponse.class);
  ...
  public UserResponse getUserByEmail(@NonNull final String email) {
    String url =
        UriComponentsBuilder.fromHttpUrl(getUserByEmailUrl).queryParam("email", email).toString();

    ResponseEntity<UserResponse> responseEntity =
        restTemplate.exchange(url, GET, new HttpEntity<>(null, null), UserResponse.class);
  ...
  public UserResponse getUserByPhone(@NonNull final String phone) {
    String url =
        UriComponentsBuilder.fromHttpUrl(getUserByPhoneUrl).queryParam("phone", phone).toString();
  ...
  public UserResponse saveNewUser(@NonNull final UserRequest userRequest) {
    ...
    ResponseEntity<UserResponse> responseEntity =
        restTemplate.exchange(
            saveNewUserUrl, POST, new HttpEntity<>(userRequest, httpHeaders), UserResponse.class);

    return responseEntity.getBody();
  }

  public UserResponse updateUser(@NonNull final String id, @NonNull final UserRequest userRequest) {
    String url = UriComponentsBuilder.fromHttpUrl(updateUserPutUrl).buildAndExpand(id).toString();

    ResponseEntity<UserResponse> responseEntity =
        restTemplate.exchange(url, PUT, new HttpEntity<>(userRequest, null), UserResponse.class);

    return responseEntity.getBody();
  }

  public UserResponse updateUser(
      @NonNull final String id, @NonNull final Map<String, Object> newValues) {
    String url = UriComponentsBuilder.fromHttpUrl(updateUserPatchUrl).buildAndExpand(id).toString();

    ResponseEntity<UserResponse> responseEntity =
        restTemplate.exchange(url, PATCH, new HttpEntity<>(newValues, null), UserResponse.class);
