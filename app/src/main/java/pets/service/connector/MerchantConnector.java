...
public class MerchantConnector {
  ...
  public RefMerchantResponse getMerchantById(@NonNull final String id) {
    String url = UriComponentsBuilder.fromHttpUrl(getMerchantByIdUrl).buildAndExpand(id).toString();

    ResponseEntity<RefMerchantResponse> responseEntity =
        restTemplate.getForEntity(url, RefMerchantResponse.class);
  ...
  public RefMerchantResponse getMerchantsByUser(@NonNull final String username) {
    String url =
        UriComponentsBuilder.fromHttpUrl(getMerchantsByUserUrl).buildAndExpand(username).toString();
  ...
  public RefMerchantResponse updateMerchant(
      @NonNull final String id, @NonNull final RefMerchantRequest merchantRequest) {
    String url = UriComponentsBuilder.fromHttpUrl(updateMerchantUrl).buildAndExpand(id).toString();

    ResponseEntity<RefMerchantResponse> responseEntity =
        restTemplate.exchange(
            url, PUT, new HttpEntity<>(merchantRequest, null), RefMerchantResponse.class);

    return responseEntity.getBody();
  }

  public RefMerchantResponse deleteMerchant(@NonNull final String id) {
    String url = UriComponentsBuilder.fromHttpUrl(deleteMerchantUrl).buildAndExpand(id).toString();

    ResponseEntity<RefMerchantResponse> responseEntity =
        restTemplate.exchange(url, DELETE, new HttpEntity<>(null, null), RefMerchantResponse.class);
