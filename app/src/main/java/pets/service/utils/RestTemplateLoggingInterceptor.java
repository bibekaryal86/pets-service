...
public class RestTemplateLoggingInterceptor implements ClientHttpRequestInterceptor {
    ...
    private void logRequest(HttpRequest request, byte[] body) {
        ...
        StringBuilder stringBuilder = new StringBuilder("Sending [").append(request.getMethod())
    ...
    private void logResponse(HttpRequest request, ClientHttpResponse clientHttpResponse, long durationInMs) {
        String maskedUri = maskDetails(request.getURI().toString());
        ...
        try {
            ...
            if (contentLength != 0L) {
                if (this.hasTextBody(httpHeaders)) {
                    ...
                    stringBuilder.append(" [Content Type] [ ").append(httpHeaders.getContentType()).append(" ]");
            ...
            this.responseLogger.error("Failed to Log Response for {} Request to {}", request.getMethod(), maskedUri, ex);
        }
    }
