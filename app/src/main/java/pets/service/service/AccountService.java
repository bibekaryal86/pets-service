...
public class AccountService {
  ...
  public AccountResponse getAccountsByUser(
      String username, AccountFilters accountFilters, boolean applyAllDetails) {
    AccountResponse accountResponse;
    ...
    try {
      ...
      logger.error(
          "Exception in Get Accounts By User: {} | {} | {}",
          username,
          accountFilters,
          applyAllDetails);
  ...
  public AccountResponse saveNewAccount(
      String username, AccountRequest accountRequest, boolean applyAllDetails) {
    AccountResponse accountResponse;
    ...
    try {
      ...
      logger.error(
          "Exception in Save New Account: {} | {} | {}", username, accountRequest, applyAllDetails);
  ...
  public AccountResponse updateAccount(
      String username, String id, AccountRequest accountRequest, boolean applyAllDetails) {
    AccountResponse accountResponse;
    ...
    try {
      ...
      if (accountResponse.getDeleteCount().intValue() > 0) {
        ...
        if (transactionResponse.getStatus() != null) {
          accountResponse =
              responseException(
                  "Account Deleted! But Error Deleting Transactions!! Please Try Again!!!", null);
        }
      }
    } catch (Exception ex) {
  ...
  private AccountResponse response(String errMsg, Exception ex) {
    if (ex instanceof HttpStatusCodeException) {
      try {
        ...
        return objectMapper()
            .readValue(
                ((HttpStatusCodeException) ex).getResponseBodyAsString(), AccountResponse.class);
      } catch (Exception ex1) {
