...
public class TransactionService {
  ...
  public TransactionResponse getTransactionById(
      String username, String id, boolean applyAllDetails) {
    ...
    try {
      ...
      logger.error(
          "Exception in Get Transaction by Id: {} | {} | {}", username, id, applyAllDetails);
      transactionResponse = response("Transaction Unavailable! Please Try Again!!!", ex);
    }

    if (!isEmpty(transactionResponse.getTransactions()) && applyAllDetails) {
  ...
  public TransactionResponse getTransactionsByUser(
      String username, TransactionFilters transactionFilters, boolean applyAllDetails) {
    TransactionResponse transactionResponse;
    ...
    try {
      ...
      logger.error(
          "Exception in Get Transactions By User: {} | {} | {}",
          username,
          transactionFilters,
          applyAllDetails);
  ...
  public TransactionResponse saveNewTransaction(
      String username, TransactionRequest transactionRequest, boolean applyAllDetails) {
    TransactionResponse transactionResponse;
    ...
    if (hasText(transactionRequest.getNewMerchant())) {
      ...
      RefMerchantResponse refMerchantResponse =
          merchantService.saveNewMerchant(
              RefMerchantRequest.builder()
                  .username(username)
      ...
      if (refMerchantResponse.getStatus() == null
        ...
        logger.error(
            "Error Saving New Merchant for Transaction: {} | {}",
            username,
            transactionRequest.getNewMerchant());
    ...
    try {
      ...
      logger.error(
          "Exception in Save New Transaction: {} | {} | {}",
          username,
          transactionRequest,
          applyAllDetails);
  ...
  public TransactionResponse updateTransaction(
      String username, String id, TransactionRequest transactionRequest, boolean applyAllDetails) {
    TransactionResponse transactionResponse;

    // check if user selected to enter a new merchant
    if (hasText(transactionRequest.getNewMerchant())) {
      // save merchant first and set it as merchant id in transaction request
      RefMerchantResponse merchantResponse =
          merchantService.saveNewMerchant(
              RefMerchantRequest.builder()
                  .username(username)
                  .description(transactionRequest.getNewMerchant())
      ...
      if (merchantResponse.getStatus() == null && !isEmpty(merchantResponse.getRefMerchants())) {
  ...
  private void applyAllDetails(String username, TransactionResponse transactionResponse) {
    ...
    CompletableFuture<RefMerchantResponse> refMerchantResponseCompletableFuture =
        merchantService.getMerchantsByUserFuture(username);
    CompletableFuture<RefTransactionTypeResponse> refTransactionTypeResponseCompletableFuture =
        transactionTypeService.getAllTransactionTypesFuture();

    applyAllDetailsStatic(
        transactionResponse,
        accountResponseCompletableFuture,
        refCategoryResponseCompletableFuture,
        refMerchantResponseCompletableFuture,
