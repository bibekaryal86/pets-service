...
public class TransactionHelper {
  ...
  public static TransactionResponse applyFilters(
      TransactionResponse transactionResponse, TransactionFilters transactionFilters) {
    ...
    if (hasText(transactionFilters.getAccountTypeId())) {
    ...
    if (hasText(transactionFilters.getBankId())) {
    ...
    if (hasText(transactionFilters.getDateFrom())) {
      transactions = applyDateFilterFrom(transactions, transactionFilters.getDateFrom());
    }

    if (hasText(transactionFilters.getDateTo())) {
  ...
  public static void applyAllDetailsStatic(
      ...
      @NonNull
          CompletableFuture<RefTransactionTypeResponse>
              refTransactionTypeResponseCompletableFuture) {
  ...
  private static List<Transaction> applyAccountFilter(
      List<Transaction> transactions, String accountId) {
    List<Transaction> fromAccountId =
        transactions.stream()
            .filter(transaction -> transaction.getAccount().getId().equals(accountId))
            .collect(toList());
    List<Transaction> fromTrfAccountId =
        transactions.stream()
  ...
  private static List<Transaction> applyAccountTypeFilter(
      List<Transaction> transactions, String accountTypeId) {
    return transactions.stream()
        .filter(
            transaction ->
                transaction.getAccount().getRefAccountType().getId().equals(accountTypeId))
        .collect(toList());
  }

  private static List<Transaction> applyAmountFilterFrom(
      List<Transaction> transactions, BigDecimal amountFrom) {
    return transactions.stream()
        .filter(transaction -> transaction.getAmount().compareTo(amountFrom) >= 0)
        .collect(toList());
  }

  private static List<Transaction> applyAmountFilterTo(
      List<Transaction> transactions, BigDecimal amountTo) {
    return transactions.stream()
  ...
  private static List<Transaction> applyCategoryFilter(
      List<Transaction> transactions, String categoryId) {
    return transactions.stream()
        .filter(transaction -> transaction.getRefCategory().getId().equals(categoryId))
        .collect(toList());
  }

  private static List<Transaction> applyCategoryTypeFilter(
      List<Transaction> transactions, String categoryTypeId) {
    return transactions.stream()
        .filter(
            transaction ->
                transaction.getRefCategory().getRefCategoryType().getId().equals(categoryTypeId))
        .collect(toList());
  }

  private static List<Transaction> applyDateFilterFrom(
      List<Transaction> transactions, String dateFrom) {
    return transactions.stream()
        .filter(
            transaction ->
                LocalDate.parse(transaction.getDate()).compareTo(LocalDate.parse(dateFrom)) >= 0)
        .collect(toList());
  }

  private static List<Transaction> applyDateFilterTo(
      List<Transaction> transactions, String dateTo) {
    return transactions.stream()
        .filter(
            transaction ->
                LocalDate.parse(transaction.getDate()).compareTo(LocalDate.parse(dateTo)) <= 0)
        .collect(toList());
  }

  private static List<Transaction> applyMerchantFilter(
      List<Transaction> transactions, String merchantId) {
    return transactions.stream()
        .filter(transaction -> transaction.getRefMerchant().getId().equals(merchantId))
        .collect(toList());
  }

  private static List<Transaction> applyNecessaryFilter(
      List<Transaction> transactions, boolean necessary) {
    return transactions.stream()
        .filter(
            transaction ->
  ...
  private static List<Transaction> applyRegularFilter(
      List<Transaction> transactions, boolean regular) {
    return transactions.stream()
        .filter(
            transaction -> transaction.getRegular() != null && transaction.getRegular() == regular)
        .collect(toList());
  }

  private static List<Transaction> applyTypeFilter(
      List<Transaction> transactions, String transactionTypeId) {
    return transactions.stream()
        .filter(
            transaction -> transaction.getRefTransactionType().getId().equals(transactionTypeId))
        .collect(toList());
  }

  private static void applyAccountDetails(
      TransactionResponse transactionResponse,
  ...
  private static void applyAccountDetails(
      TransactionResponse transactionResponse, @NonNull AccountResponse accountResponse) {
    accountResponse
        .getAccounts()
        .forEach(
            account ->
                    ...
                    .filter(
                        transaction ->
                            transaction.getAccount() != null
                                && hasText(transaction.getAccount().getId())
                                && hasText(account.getId()))
  ...
  private static void applyTrfAccountDetails(
      TransactionResponse transactionResponse, @NonNull AccountResponse accountResponse) {
    accountResponse
        .getAccounts()
        .forEach(
                    ...
                    .filter(
                        transaction ->
                            transaction.getTrfAccount() != null
                                ...
                                && hasText(account.getId()))
                    .filter(
                        transaction -> transaction.getTrfAccount().getId().equals(account.getId()))
                    .forEach(
                        transaction -> {
  ...
  private static void applyCategoryDetails(
      TransactionResponse transactionResponse,
  ...
  private static void applyCategoryDetails(
      TransactionResponse transactionResponse, @NonNull RefCategoryResponse refCategoryResponse) {
    refCategoryResponse
        .getRefCategories()
        .forEach(
            category ->
                    ...
                    .filter(
                        transaction ->
                            transaction.getRefCategory() != null
                                && hasText(transaction.getRefCategory().getId())
                                && hasText(category.getId()))
                    .filter(
                        transaction ->
                            transaction.getRefCategory().getId().equals(category.getId()))
                    .forEach(
                        transaction -> {
                          transaction.getRefCategory().setDescription(category.getDescription());
                          transaction
                              .getRefCategory()
                              .setRefCategoryType(category.getRefCategoryType());
                        }));
  }

  private static void applyMerchantDetails(
      TransactionResponse transactionResponse,
  ...
  private static void applyMerchantDetails(
      TransactionResponse transactionResponse, @NonNull RefMerchantResponse refMerchantResponse) {
    refMerchantResponse
        .getRefMerchants()
        .forEach(
            merchant ->
                transactionResponse.getTransactions().stream()
                    .filter(
                        transaction ->
                            transaction.getRefMerchant() != null
                                && hasText(transaction.getRefMerchant().getId())
                                && hasText(merchant.getId()))
                    .filter(
                        transaction ->
                            transaction.getRefMerchant().getId().equals(merchant.getId()))
                    .forEach(
                        transaction ->
                            transaction
                                .getRefMerchant()
  ...
  private static void applyTransactionTypeDetails(
      TransactionResponse transactionResponse,
      CompletableFuture<RefTransactionTypeResponse> refTransactionTypeResponseCompletableFuture) {
    applyTransactionTypeDetails(
        transactionResponse, refTransactionTypeResponseCompletableFuture.join());
  }

  private static void applyTransactionTypeDetails(
      TransactionResponse transactionResponse,
      @NonNull RefTransactionTypeResponse refTransactionTypeResponse) {
                    ...
                    .filter(
                        transaction ->
                            transaction.getRefTransactionType() != null
                                && hasText(transaction.getRefTransactionType().getId())
                                && hasText(transactionType.getId()))
                    .filter(
                        transaction ->
                            transaction
                                .getRefTransactionType()
                                .getId()
                                .equals(transactionType.getId()))
                    .forEach(
                        transaction ->
                            transaction
                                .getRefTransactionType()
