...
public class ReportsService {
  ...
  public ReportsService(AccountService accountService, TransactionService transactionService) {
  ...
  public ReportsResponse getCurrentBalancesReport(String username) {
    ...
    if (accountResponse.getStatus() == null) {
      ...
      reportsResponse =
          reportResponseError("Current Balances", accountResponse.getStatus().getErrMsg());
    }
  ...
  private List<ReportCurrentBalances> calculateCurrentBalances(@NonNull List<Account> accounts) {
    ...
    BigDecimal totalCheckingAccounts =
        accounts.stream()
            .filter(account -> account.getRefAccountType().getId().equals(ACCOUNT_TYPE_ID_CHECKING))
            .map(Account::getCurrentBalance)
            .reduce(BigDecimal.ZERO, BigDecimal::add);
    BigDecimal totalSavingsAccounts =
        accounts.stream()
            .filter(account -> account.getRefAccountType().getId().equals(ACCOUNT_TYPE_ID_SAVINGS))
            .map(Account::getCurrentBalance)
            .reduce(BigDecimal.ZERO, BigDecimal::add);
    BigDecimal totalInvestmentAccounts =
        accounts.stream()
            .filter(
                account -> account.getRefAccountType().getId().equals(ACCOUNT_TYPE_ID_INVESTMENT))
            .map(Account::getCurrentBalance)
            .reduce(BigDecimal.ZERO, BigDecimal::add);
    BigDecimal totalOtherDepositAccounts =
        accounts.stream()
            .filter(
                account ->
                    account.getRefAccountType().getId().equals(ACCOUNT_TYPE_ID_OTHER_DEPOSITS))
            .map(Account::getCurrentBalance)
            .reduce(BigDecimal.ZERO, BigDecimal::add);
    BigDecimal totalCreditCards =
        accounts.stream()
            .filter(
                account -> account.getRefAccountType().getId().equals(ACCOUNT_TYPE_ID_CREDIT_CARD))
            .map(Account::getCurrentBalance)
            .reduce(BigDecimal.ZERO, BigDecimal::add);
    BigDecimal totalLoansAndMortgages =
        accounts.stream()
            .filter(
                account ->
                    account.getRefAccountType().getId().equals(ACCOUNT_TYPE_ID_LOANS_MORTGAGES))
            .map(Account::getCurrentBalance)
            .reduce(BigDecimal.ZERO, BigDecimal::add);
    BigDecimal totalOtherLoanAccounts =
        accounts.stream()
            .filter(
                account -> account.getRefAccountType().getId().equals(ACCOUNT_TYPE_ID_OTHER_LOANS))
            .map(Account::getCurrentBalance)
            .reduce(BigDecimal.ZERO, BigDecimal::add);
    BigDecimal totalAssets =
        totalCash
            .add(totalCheckingAccounts)
            .add(totalSavingsAccounts)
            .add(totalInvestmentAccounts)
            .add(totalOtherDepositAccounts);
    BigDecimal totalDebts =
        totalCreditCards.add(totalLoansAndMortgages).add(totalOtherLoanAccounts);
  ...
  public ReportsResponse getCashFlowsReport(String username, String selectedYear) {
    ...
    TransactionResponse transactionResponse =
        transactionService.getTransactionsByUser(username, transactionFilters, false);

    if (transactionResponse.getStatus() == null) {
      reportsResponse =
          ReportsResponse.builder()
              .reportCashFlows(
                  calculateCashFlows(transactionResponse.getTransactions(), selectedYear))
              .build();
    } else {
      reportsResponse =
          reportResponseError("Cash Flows", transactionResponse.getStatus().getErrMsg());
    }

    return reportsResponse;
  }

  private List<ReportCashFlows> calculateCashFlows(
      @NonNull List<Transaction> transactions, String selectedYear) {
        ...
        month -> {
          List<Transaction> monthlyTransactions =
              transactions.stream()
                  ...
                  .collect(toList());
          reportCashFlows.add(
              calculateCashFlow(monthlyTransactions, month, Integer.parseInt(selectedYear)));
        });
  ...
  private ReportCashFlows calculateCashFlow(
      List<Transaction> monthlyTransactions, Month month, int selectedYear) {
    BigDecimal totalIncomes =
        monthlyTransactions.stream()
            .filter(
                transaction ->
                    transaction.getRefTransactionType().getId().equals(TRANSACTION_TYPE_ID_INCOME)
    ...
    BigDecimal totalExpenses =
        monthlyTransactions.stream()
            .filter(
                transaction ->
                    transaction.getRefTransactionType().getId().equals(TRANSACTION_TYPE_ID_EXPENSE)
                        || (transaction
                                .getRefTransactionType()
                                .getId()
                                .equals(TRANSACTION_TYPE_ID_TRANSFER)
                            && CATEGORY_ID_LOAN_PAYMENTS.contains(
                                transaction.getRefCategory().getId())))
            .map(Transaction::getAmount)
            .reduce(BigDecimal.ZERO, BigDecimal::add);
    ...
    String monthValue =
        month.getValue() < 10
            ? String.format("0%s", month.getValue())
  ...
  public ReportsResponse getCategoriesReport(String username, String selectedYear) {
    ...
    TransactionFilters transactionFilters =
        TransactionFilters.builder()
            ...
            .build();
    TransactionResponse transactionResponse =
        transactionService.getTransactionsByUser(username, transactionFilters, true);

    if (transactionResponse.getStatus() == null) {
      reportsResponse =
          ReportsResponse.builder()
              .reportCategoryTypes(
                  calculateCategoryTypesTotals(transactionResponse.getTransactions()))
              .build();
    } else {
      reportsResponse =
          reportResponseError("Categories", transactionResponse.getStatus().getErrMsg());
    }

    return reportsResponse;
  }

  private List<ReportCategoryTypes> calculateCategoryTypesTotals(
      @NonNull List<Transaction> transactions) {
    List<ReportCategoryTypes> reportCategoryTypes = new ArrayList<>();

    Set<RefCategoryType> refCategoryTypes =
        transactions.stream()
            .filter(
                transaction ->
                    !transaction
    ...
    refCategoryTypes.forEach(
        refCategoryType -> {
          List<Transaction> categoryTypesTransactions =
              transactions.stream()
                  .filter(
                      transaction ->
                          transaction
                              ...
                              .getRefCategoryType()
                              .getId()
                              .equals(refCategoryType.getId()))
                  .collect(toList());
          reportCategoryTypes.add(
              calculateCategoryTypesTotals(categoryTypesTransactions, refCategoryType));
        });

    reportCategoryTypes.sort(
        comparing(reportCategoryType -> reportCategoryType.getRefCategoryType().getDescription()));

    return reportCategoryTypes;
  }

  private ReportCategoryTypes calculateCategoryTypesTotals(
      List<Transaction> categoryTypeTransactions, RefCategoryType refCategoryType) {
    BigDecimal totalCategoryType =
        categoryTypeTransactions.stream()
            .filter(
                transaction ->
                    transaction
                        .getRefCategory()
  ...
  private List<ReportCategories> calculateCategoriesTotals(
      List<Transaction> categoryTypeTransactions) {
    List<ReportCategories> reportCategories = new ArrayList<>();

    Set<RefCategory> refCategories =
        categoryTypeTransactions.stream().map(Transaction::getRefCategory).collect(toSet());

    refCategories.forEach(
        refCategory -> {
          List<Transaction> categoryTransactions =
              categoryTypeTransactions.stream()
                  .filter(
                      transaction ->
                          transaction.getRefCategory().getId().equals(refCategory.getId()))
                  .collect(toList());
          reportCategories.add(calculateCategoriesTotals(categoryTransactions, refCategory));
        });

    reportCategories.sort(
        comparing(reportCategory -> reportCategory.getRefCategory().getDescription()));

    return reportCategories;
  }

  private ReportCategories calculateCategoriesTotals(
      List<Transaction> categoryTransactions, RefCategory refCategory) {
    BigDecimal totalCategory =
    ...
    return ReportsResponse.builder()
        .status(
            Status.builder()
                .errMsg(
                    String.format(
                        "Something Went Wrong! Error Retrieving %s Report!! Please Try Again!!!",
                        errMsg))
                .message(message)
                .build())
        .build();
