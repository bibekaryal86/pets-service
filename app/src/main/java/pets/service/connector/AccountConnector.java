...
public class AccountConnector {
  ...
  public AccountResponse getAccountById(@NonNull final String id) {
    String url = UriComponentsBuilder.fromHttpUrl(getAccountByIdUrl).buildAndExpand(id).toString();

    ResponseEntity<AccountResponse> responseEntity =
        restTemplate.getForEntity(url, AccountResponse.class);
  ...
  public AccountResponse getAccountsByUser(@NonNull final String username) {
    String url =
        UriComponentsBuilder.fromHttpUrl(getAccountsByUserUrl).buildAndExpand(username).toString();
  ...
  public AccountResponse saveNewAccount(@NonNull final AccountRequest accountRequest) {
    ResponseEntity<AccountResponse> responseEntity =
        restTemplate.exchange(
            saveNewAccountUrl, POST, new HttpEntity<>(accountRequest, null), AccountResponse.class);

    return responseEntity.getBody();
  }

  public AccountResponse updateAccount(
      @NonNull final String id, @NonNull final AccountRequest accountRequest) {
    String url =
        UriComponentsBuilder.fromHttpUrl(updateAccountPutUrl).buildAndExpand(id).toString();

    ResponseEntity<AccountResponse> responseEntity =
        restTemplate.exchange(
            url, PUT, new HttpEntity<>(accountRequest, null), AccountResponse.class);

    return responseEntity.getBody();
  }

  public AccountResponse deleteAccount(@NonNull final String id) {
    String url = UriComponentsBuilder.fromHttpUrl(deleteAccountUrl).buildAndExpand(id).toString();

    ResponseEntity<AccountResponse> responseEntity =
        restTemplate.exchange(url, DELETE, new HttpEntity<>(null, null), AccountResponse.class);
